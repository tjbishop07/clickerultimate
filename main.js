...
function gameLoop(exhaustTick) {
  ...
  if (game.global.autoClicking && game.player.autoClick > 1) {
    ...
    if (game.global.autoClicking == "start") {
      ...
      game.global.autoClicking === true &&
      (loops - tempLoop) % autoClickRate == 0
    ) {
...
function toggleExport() {
  if (game.global.currentTooltip == "export" && game.global.isPaused)
    tooltip("pause");
...
function toggleImport() {
  if (game.global.currentTooltip == "import" && game.global.isPaused)
    tooltip("pause");
...
function toggleDeleteSave() {
  ...
  if (game.settings.hardResetExposed)
    message(
      "The Delete Save button is now exposed in the bottom ribbon.",
      "warning"
    );
}
...
function updateDeleteSaveSetting() {
  // document.getElementById("mnuCurrentDeleteSave").innerHTML = game.settings
  //   .hardResetExposed
  //   ? "Exposed"
  //   : "Hidden";
  // if (game.settings.hardResetExposed) {
  ...
  //   document.getElementById("deleteSaveButton").onclick = function () {
  //     tooltip("delete");
  //   };
  // } else {
  //   document.getElementById("deleteSaveButton").innerHTML = "";
...
function toggleTutorial() {
  ...
  if (game.settings.tutorialCompleted && game.settings.tutorialEnabled)
...
function updateTutorialSetting() {
  document.getElementById("mnuCurrentTutorial").innerHTML = game.settings
    .tutorialEnabled
    ? "Enabled"
    : "Disabled";
}
...
function toggleGreeting() {
  ...
  game.settings.greetingMessage = toggleOption(
    availableGreetings,
    game.settings.greetingMessage
  );
  updateGreeting();
}
...
function updateGreeting() {
  document.getElementById("mnuCurrentLatestChanges").innerHTML =
    game.settings.greetingMessage;
}
...
function updateRoundNumberSetting() {
  document.getElementById("mnuCurrentRounding").innerHTML = game.settings
    .roundResources
    ? "Yes"
    : "No";
}
...
function updateDecimalPlacesSetting() {
  document.getElementById("mnuCurrentDecimals").innerHTML =
    (game.settings.resourceAlwaysDecimals ? "Always " : "") +
    game.settings.resourceDecimals;
  for (var res in game.resources) {
...
function toggleDefaultMenu() {
  ...
  game.settings.defaultMenu = toggleOption(
    availableMenuOptions,
    game.settings.defaultMenu
  );
  updateDefaultMenu();
}
...
function updateDefaultMenu() {
  document.getElementById("mnuCurrentDefaultMenu").innerHTML =
    game.settings.defaultMenu;
}
...
function toggleMsgFontSize() {
  ...
  game.settings.messagesSize = toggleOption(
    availableSizes,
    game.settings.messagesSize
  );
  updateMsgFontSize();
}
...
function updateMsgFontSize() {
  var msgClass = "";
  document
    .getElementById("topMessagesContainer")
    .classList.remove(
      "msgFontSmall",
      "msgFontNormal",
      "msgFontLarge",
  ...
  document.getElementById("topMessagesContainer").classList.add(msgClass);
  document.getElementById("mnuCurrentMsgSize").innerHTML =
    game.settings.messagesSize;
}
...
function toggleTooltipModes() {
  ...
  game.settings.tooltipMode = toggleOption(
    availableTooltipModes,
    game.settings.tooltipMode
  );
  updateTooltipMode();
}
...
function updateTooltipMode() {
  document.getElementById("mnuCurrentLimitedTooltip").innerHTML =
    game.settings.tooltipMode;
}
...
function updateAutoskipBeginning() {
  document.getElementById("mnuCurrentAutoskipBeginning").innerHTML = game
    .settings.skipFirstUpg
    ? "Skip Beginning"
    : "Play Beginning";
}
...
function leaderGreeting(leader) {
  ...
  switch (leader.personality) {
    case "wise":
      messages = [
        "Do you seek my counsel? Very well...",
        "May we help " + empire + " prosper.",
        "I foresaw our meeting in a dream.",
        "Wise of you to heed my counsel, lord.",
        Empire + " is destined for greatness with me at your side.",
        "Long may you reign over " + empire + ".",
      ];
      break;
    case "stern":
      messages = [
    ...
    case "pious":
      messages = [
        ...
        "God watches over us.",
        "By the grace of God.",
        "We shall strive to earn God's favor.",
      ];
      break;
    default:
...
function leaderAdvice(age) {
  ...
  for (var ldr in game.leaders) {
    ...
    for (var adv in game.advancements) {
      ...
      if (advancement.parent != age || advancement.locked || advancement.bought)
        continue;
      if (
        (!leader.favored || !leader.favored.includes(adv)) &&
        (!leader.unfavored || !leader.unfavored.includes(adv))
      )
        continue;
      leaderList.push({
        leader: leader.label,
  ...
  switch (leaderPick.personality) {
    case "wise":
      if (leaderPick.position == "favor") {
        messages = [
          "It would be wise of you to research <b>" +
            leaderPick.advancement +
            "</b>.",
          "May I recommend researching <b>" + leaderPick.advancement + "</b>?",
        ];
      } else {
        messages = [
          "I do not think " +
            empire +
            " would benefit from <b>" +
            leaderPick.advancement +
            "</b>.",
          "I suggest avoiding the <b>" +
            leaderPick.advancement +
            "</b> advancement.",
        ];
      }
    ...
    case "stern":
      if (leaderPick.position == "favor") {
        messages = [
          "No doubt <b>" +
            leaderPick.advancement +
            "</b> would be worth the cost.",
          Empire +
            " would be better off with the <b>" +
            leaderPick.advancement +
            "</b> advancement.",
          "I urge you to consider purchasing <b>" +
            leaderPick.advancement +
            "</b>.",
        ];
      } else {
        messages = [
          "<b>" + leaderPick.advancement + "</b> seems like a waste of time.",
          "I advise against researching <b>" + leaderPick.advancement + "</b>.",
        ];
      }
      break;
    case "pious":
      if (leaderPick.position == "favor") {
        messages = [
          "I sense the <b>" +
            leaderPick.advancement +
            "</b> advancement has a lot of potential.",
          "My liege, <b>" +
            leaderPick.advancement +
            "</b> would change " +
            empire +
        ...
        messages = [
          "I see no reason to invest in <b>" + leaderPick.advancement + "</b>.",
          "Wasting points on the <b>" +
            leaderPick.advancement +
            "</b> advancement would be a sin.",
        ];
      }
      break;
    default:
      if (leaderPick.position == "favor") {
        messages = [
          "<b>" + leaderPick.advancement + "</b> seems like a wise investment.",
          Empire + " could use <b>" + leaderPick.advancement + "</b>.",
        ];
      } else {
        messages = [
          "This <b>" +
            leaderPick.advancement +
            "</b> seems like a poor investment.",
          "I advise against choosing <b>" + leaderPick.advancement + "</b>.",
        ];
      }
  }
...
function leaderMessage(advancement) {
  ...
  for (var ldr in game.leaders) {
    if (!game.leaders[ldr].bought) continue;
    if (
      game.leaders[ldr].favored &&
      game.leaders[ldr].favored.includes(advancement.name)
    ) {
      leaderList.push(ldr);
      favoringList.push("favor");
    } else if (
      game.leaders[ldr].unfavored &&
      game.leaders[ldr].unfavored.includes(advancement.name)
    ) {
      leaderList.push(ldr);
  ...
  if (favor == "favor") {
    availableMessages = [
      ...
      "I believe <b>" +
        advancement.label +
        "</b> will play an important role in " +
        empire +
        ".",
      "<b>" +
        advancement.label +
        "</b> will change the course of history for the better.",
    ];
  } else {
    availableMessages = [
      "I do not think <b>" +
        advancement.label +
        "</b> will be beneficial in the long run.",
      "Was <b>" + advancement.label + "</b> really a wise investment?.",
      "<b>" +
        advancement.label +
        "</b> clearly was not worth the advancement points.",
      "I think there were better advancements to research than <b>" +
        advancement.label +
        "</b>.",
  ...
  message(
    "<b>" + leader.label + '</b>: "' + getRandom(availableMessages) + '"'
  );
}
...
function tutorialMessage(type) {
  ...
  var tutorialMessages = {
    ...
    gather:
      "Click on the <b>Gather</b> button above to fill the Progress Bar of a resource. Doing so will grant you resources which you can use to buy things with.",
    workers:
      "You can hover over the tile of a <b>Worker</b> to see what it does and how much it costs. The more workers you have, the more their effects stack!",
    workers2:
      "Some <b>Workers</b> have a constant cost to some resources. Be sure to read their descriptions before buying them.",
    resources:
      "Did you know? If you click on the name of a resource, you can get more information about what affects it and how you are collecting it.",
    upgrades:
      "Plan your <b>Upgrades</b> ahead of time so you know which resources' rates to improve upon. Buying the first available <b>Upgrade</b> may not always be the answer.",
    levels:
      "Some <b>Upgrades</b> have several <b>Tiers</b>, meaning they can be bought more than once. When hovering over the tile of an <b>Upgrade</b>, look for numbers right of the upgrade's name indicating which tier you are looking at.",
    advancements:
      "You can use <b>Advancement Points</b> to purchase <b>Advancements</b>, which are like <b>Upgrades</b> but better! Hover over them to see if any of them interest you.",
    ages:
      "Each time you advance to a new <b>Age</b>, you unlock <b>Advancements</b>. Keeping your <b>Advancement Points</b> for later, or buying the ones from previous ages are both valid strategies.",
    buildings:
      "<b>Buildings</b> are similar to <b>Workers</b>, but they typically bypass the <b>Progress Bar</b> by directly giving raw resources. Therefore the rate should not be the only thing to consider when deciding whether to buy a worker or a building.",
    highprogress:
      "Some resources are not always worth clicking your way through. Try to look for <b>Workers</b>, <b>Buildings</b> or <b>Upgrades</b> as alternatives.",
    themes:
      "You can change the look of the interface by changing the current <b>Theme</b> in the <b>Settings</b> menu. Give it a try!",
    trades:
      "<b>Trades</b> are exchanges of goods that can be repeatedly done to stimulate your economy. Some trades will require resources, other gold, or a mix of both. Make sure to have a look at the available trades when struggling with a particular resource.",
    tax:
      "Try to balance your ratio of workers-to-buildings to get the most out of your <b>Tax</b> and <b>Property Tax</b> trades. <b>Gold</b> is worth the trouble!",
    prestige:
      "The <b>Prestige</b> menu allows you to set out and leave your civilization behind in favor of building a <b>Colony</b>. Doing so means you will have to start from scratch, but you will be allowed to carry over <b>Prestige Points</b> and" +
      " <b>Advancement Points</b>. Prestige points can be used to purchase permanent bonuses that stick from colony to colony.",
    prestige2:
      "The further you go with your civilization, the more <b>Prestige Points</b> you gain. It's all about finding the right balance between progressing further and starting a new colony!",
    prestige3:
      "The more <b>Colonies</b> you build, the faster you get at reaching the next colony. On top of that, you may also discover <b>Secret Upgrades</b> as you create more colonies.",
    tutorial:
      "You can disable these tips by switching the tutorial off in the Settings.",
    leaders:
      "<b>Leaders</b> are unique characters with various benefits. It's worth mentioning you can only hire <b>one leader</b> for every colony you build, so better think twice before making your decision.",
    leaders2:
      "Don't be too eager to hire a <b>Leader</b>, as you will unlock more and more as your civilization progresses. As with other things, patience is a virtue.",
    leaders3:
      "<b>Leaders</b> you unlock stay available for hire on your subsequent colonies. Additionally, some <b>Leaders</b> can only be unlocked under special circumstances, so don't be afraid to experiment.",
  };

  var item = type;
  var availableTypes = tutorialMessages.available;
  if (game.upgrades.upgLeaders.bought)
    availableTypes.push("leaders2", "leaders3");
  if (!item) {
 ...
 */
function randomTip() {
  if (!game.settings.tutorialEnabled || !game.settings.tutorialCompleted)
    return;
  tutorialMessage();
  ...
  switch (game.settings.greetingMessage) {
    case "Latest Changes":
      msg =
        "Welcome to <b>v" +
        game.global.version +
        "</b>. Latest changes are as follows: <i>" +
        game.global.latestChanges +
        "</i>";
      break;
    case "Version Number":
      msg =
        "Welcome to <b>Clicker Ultimate</b> (v" + game.global.version + ").";
      break;
    case "Simple":
...
function updateUnreadMessages() {
  ...
  var number =
    game.global.unreadMessages <= 0 ? "" : game.global.unreadMessages;
  if (number != "")
    number =
 ...
 */
function importLoad() {
  var saveString = document.getElementById("importText")
    ? document.getElementById("importText").value
    : "";
  if (saveString.length < 1) {
    message(
      'Could not read your Import String. Please enter a valid Export String in the "Import" box in order to load your game.',
      "warning"
    );
    toggleImport();
    return;
...
function save(isExport) {
  ...
  try {
    ...
    if (localStorage.getItem("ClickerUltisvg") == saveString) {
      ...
      message(
        "Unable to save game. Please use the export feature instead to back up your progress.",
        "warning"
      );
    }
  } catch (e) {
    if (e.name == "NS_ERROR_FILE_CORRUPTED") {
      message(
        "Browser storage appears to be corrupted. Please clear your cookies from your browser and try again.",
        "warning"
      );
    } else
      message(
...
function load(saveString) {
  ...
  if (saveString) {
    ...
    try {
      ...
      message(
        "Your browser is preventing access to localStorage. Please check your browser settings to ensure that cookies are not disabled.",
        "warning"
      );
  ...
  try {
    saveGame = JSON.parse(
      LZString.decompressFromBase64(fromImport ? saveString : unparsedSave)
    );
  } catch (e) {
  ...
  oldVersion = saveGame.global.version
    ? saveGame.global.version.split(".")
    : null;
  if (
    fromImport &&
    oldVersion &&
    oldVersion.length &&
    isVersionNewer(oldVersion, game.global.version.split("."))
  ) {
    message(
      "The imported string is from a newer version of the game. Please refresh your browser to get the latest version.",
      "warning"
    );
    return false;
  }
  ...
  if (isVersionNewer([0, 2, 4], oldVersion)) {
    ...
    for (var ach in saveGame.achievements) {
      if (game.achievements[ach] && saveGame.achievements[ach].achieved)
        i += game.achievements[ach].points;
    }
...
function replaceValueBy(oldVal, newVal) {
  ...
  for (var part in oldVal) {
    if (part == "global" && typeof oldVal[part] === "object")
      newObj[part] = replaceValueBy(oldVal[part], oldVal[part]);
  ...
  for (var part in newVal) {
    if (typeof oldVal[part] === "undefined")
      newObj[part] = replaceValueBy(newVal[part], newVal[part]);
  }
  return newObj;
}
...
function getDeltaOf(val, comparison) {
  ...
  var newObj = new Object();
  for (var part in val) {
    if (part == "global" && typeof val[part] === "object")
      newObj[part] = val[part];
    else if (
      typeof val[part] === "object" &&
      typeof comparison[part] === "undefined"
    )
      newObj[part] = replaceValueBy(val[part], val[part]);
    else {
...
function isVersionNewer(newVersion, oldVersion) {
  oldVersion = [
    parseInt(oldVersion[0], 10),
    parseInt(oldVersion[1], 10),
    parseInt(oldVersion[2], 10),
  ];
  newVersion = [
    parseInt(newVersion[0], 10),
    parseInt(newVersion[1], 10),
    parseInt(newVersion[2], 10),
  ];
...
function getAdvancements() {
  var advancementList = [];
  for (var adv in game.advancements) {
    if (game.advancements[adv].locked || game.advancements[adv].bought)
      continue;
...
function clickResource(what, isAuto = false) {
  ...
  if (
    game.player.doubleClick > 0 &&
    game.global.clicks % game.player.doubleClick == 0
  )
    number *= 2;
...
function gainResource(resource, number, active = false) {
  ...
  resource.current = prettify(
    GetMax(resource, resource.current + number, hasParent(resource), active),
    2
  );
  if (!active) resource.isActive = true;
  if (resource.current != initialCurrent)
    updateResValues(resource, true, resource.current == resource.min);
}
...
function buyWorker(worker, number, forFree = false) {
  ...
  for (var res in worker.resourceCost) {
    ...
    resource.current = prettify(
      resource.current - worker.resourceCost[res].current,
      2
    );
...
function buyBuilding(building, number, forFree = false) {
  ...
  for (var res in building.resourceCost) {
    ...
    var resource = getFromText(res);
    resource.current = prettify(
      resource.current - building.resourceCost[res].current,
      2
    );
    gainResource(getFromText(res + "P"), 0.001); //in case progress is maxed, reset automatically
    updateResValues(resource, true);
...
function buyUpgrade(upgrade) {
  ...
  var isBought = false;
  if (
    typeof upgrade.level === "undefined" ||
  ...
  for (var res in upgrade.resourceCost) {
    var resource = getFromText(res);
    resource.current = prettify(
      resource.current - upgrade.resourceCost[res].current,
      2
    );
    gainResource(getFromText(res + "P"), 0.001); //in case progress is maxed, reset automatically
    updateResValues(resource, true);
...
function buyLeader(leader) {
  if (
    !leader ||
    leader.bought ||
    game.player.currentLeaders >= game.player.maxLeaders ||
    !canAfford(leader)
  )
    return;
  ...
  for (var res in leader.resourceCost) {
    var resource = getFromText(res);
    resource.current = prettify(
      resource.current - leader.resourceCost[res].current,
      2
    );
    gainResource(getFromText(res + "P"), 0.001); //in case progress is maxed, reset automatically
    updateResValues(resource, true);
...
function buyPrestige(prestige) {
  ...
  if (
    typeof prestige.level === "undefined" ||
    prestige.level >= prestige.maxLevel
  ) {
...
function buyTrade(trade) {
  ...
  for (var res in trade.resourceCost) {
    var resource = getFromText(res);
    resource.current = prettify(
      resource.current - trade.resourceCost[res].current,
      2
    );
    gainResource(getFromText(res + "P"), 0.001); //in case progress is maxed, reset automatically
    updateResValues(resource, true);
...
function sendAchievements() {
  ...
  document.getElementById("achLabel").innerHTML = achievement.fullLabel
    ? achievement.fullLabel
    : achievement.label;
  document.getElementById("achPoints").innerHTML = achievement.points
    ? achievement.points
    : 0;
...
function addPrestigePoints(number, now = false) {
  ...
  if (now) game.player.prestigePoints += number;
    ...
    game.player.prestigePoints += Math.min(
      game.player.convertPrestiges,
      number
    );
    game.player.nextPrestigePoints += Math.max(
      number - game.player.convertPrestiges,
      0
    );
    game.player.convertPrestiges -= Math.min(
      game.player.convertPrestiges,
      number
    );
  }
...
function upgradeWorkerRate(what, number, res = "") {
  ...
  for (var resGain in worker.resourceGain) {
    if (
      (worker.resourceGain[resGain].base <= 0 && number >= 0) ||
      (worker.resourceGain[resGain].base >= 0 && number <= 0)
    )
      continue;
    if (res.length > 0 && res != resGain) continue;
    worker.resourceGain[resGain].base = prettify(
      worker.resourceGain[resGain].base + Math.abs(number),
      2
    );
  }
  recalculateWorkerRate(worker);
...
function upgradeWorkerCostRate(wor, res, rate) {
  ...
  worker.resourceCost[res].rate = prettify(
    worker.resourceCost[res].rate + rate,
    2
  );
  recalculateWorkerCost(worker);
}
    ...
    for (var parentBlock in game.resources[res].parent) {
      if (parentBlock == what) {
        ...
        description +=
          ...
          (res == what + "P"
            ? "<br />" +
              prettify(child.clickGain * game.player.clickValue, 2, true) +
              " per click."
            : "") +
          (res == what + "P" && child.rate > 0
            ? "<br />" + prettify(child.rate, 2, true) + " per second."
            : "") +
          "<br />" +
          prettify(
            child.parent[parentBlock].gain * game.player.parentValue,
            2,
            true
          ) +
          " " +
  ...
  for (var psv in game.passives) {
    ...
    for (var gainBlock in game.passives[psv].resourceGain) {
      ...
      if (gainBlock == what) {
        description +=
          '<span style ="color:' +
          (passiveRate > 0 ? "darkgreen" : "darkred") +
          ';">' +
          "<br />" +
          passive.label +
          " " +
          (passiveRate > 0 ? "giving" : "costing") +
          " " +
          prettify(Math.abs(passiveRate), 2, true) +
          " " +
          game.resources[gainBlock].label +
          " per second." +
          "</span>";
        ...
        description +=
          ...
          prettify(
            (passiveRate *
              game.resources[gainBlock].parent[what].gain *
              game.player.parentValue) /
              (game.resources[gainBlock].max - game.resources[gainBlock].min),
            3,
            true
          ) +
          " " +
  ...
  for (var wor in game.workers) {
    ...
    for (var gainBlock in game.workers[wor].resourceGain) {
      ...
      if (gainBlock == what || gainBlock == whatP) {
            ...
            ? " (" +
              prettify(
                (workerRate *
                  game.resources[gainBlock].parent[what].gain *
                  game.player.parentValue) /
                  (game.resources[gainBlock].max -
                    game.resources[gainBlock].min),
                3,
                true
              ) +
              " " +
        ...
        description +=
          '<span style ="color:' +
          (workerRate > 0 ? "darkgreen" : "darkred") +
          ';">' +
          "<br />" +
          prettify(worker.current, 0, true) +
          " " +
          plural(worker) +
          " " +
          (workerRate > 0 ? "giving" : "costing") +
          " " +
          prettify(Math.abs(workerRate), 2, true) +
          " " +
          game.resources[gainBlock].label +
          " per second." +
          rawResourceConversion +
          "</span>";
      }
    }
  }
  ...
  for (var bld in game.buildings) {
    ...
    for (var gainBlock in game.buildings[bld].resourceGain) {
      if (gainBlock == what || gainBlock == whatP) {
        ...
        description +=
          '<span style ="color:' +
          (buildingRate > 0 ? "darkgreen" : "darkred") +
          ';">' +
          "<br />" +
          building.current +
          " " +
          plural(building) +
          " " +
          (buildingRate > 0 ? "giving" : "costing") +
          " " +
          Math.abs(buildingRate) +
          " " +
          game.resources[gainBlock].label +
          " per second." +
          rawResourceConversion +
...
function getFullWorkerDescription(worker) {
  ...
  for (var gainBlock in worker.resourceGain) {
    ...
    description +=
      ...
      resource.label +
      ": " +
      prettify(worker.resourceGain[gainBlock].current, 2, true) +
      "/sec";
  }
  description += "</b>";
  return description;
...
function getCostLabel(what) {
  ...
  if (what.advCost) {
    ...
    costString +=
      "<span style='color: " +
      (game.player.advancementPoints >= what.advCost ? "green" : "red") +
      ";'>";
  ...
  if (what.goldCost) {
    if (costString.length > 0) costString += ", ";
    if (game.player.gold >= what.goldCost)
      costString += "<span style='color: green;'>";
  ...
  for (var res in what.resourceCost) {
    var resource = getFromText(res);
    if (costString.length > 0) costString += ", ";
    if (resource.current >= what.resourceCost[res].current)
      costString += "<span style='color: green;'>";
    else costString += "<span style='color: red;'>";
...
function getRequirementsLabel(what) {
  ...
  for (var req in what.requirements) {
    ...
    if (target.level >= what.requirements[req])
      reqString += "<span style='color: green;'>";
...
function unlock(...what) {
  ...
  for (var i = 0; i < what.length; i++) {
    ...
    } else if (itemType == "leaders" && !game.upgrades.upgLeaders.bought)
      continue;
...
function unlockUI(what) {
  ...
  if (resourceElem && resourceElem.classList.contains("locked"))
    resourceElem.classList.remove("locked");
}
...
function updateContainer(what) {
  ...
  switch (what) {
    ...
    case "workers":
      ...
      for (var wor in game.workers) {
        ...
        elems = [
          "WorkersTitle",
          "WorkersContainer",
          "mnuStatsLabelWorkers",
          "mnuStatsCurrentWorkers",
          "mnuStatsLabelWorkerCost",
          "mnuStatsCurrentWorkerCost",
        ];
        addWorkerBlock(wor);
      }
      break;
    case "buildings":
      ...
      for (var bld in game.buildings) {
        ...
        elems = [
          "BuildingsTitle",
          "BuildingsContainer",
          "mnuStatsLabelBuildings",
          "mnuStatsCurrentBuildings",
          "mnuStatsLabelBuildingCost",
          "mnuStatsCurrentBuildingCost",
        ];
    ...
    case "leaders":
      ...
      for (var ldr in game.leaders) {
        ...
        elems = [
          "LeadersTitle",
          "LeadersContainer",
          "mnuStatsLabelLeaders",
          "mnuStatsCurrentLeaders",
        ];
        addLeaderBlock(ldr);
      }
      break;
    case "advancements":
      ...
      break;
    case "prestiges":
      document.getElementById("PrestigeActiveContainer").innerHTML =
        "<!-- Insert active prestiges here -->";
      document.getElementById("PrestigePassiveContainer").innerHTML =
        "<!-- Insert passive prestiges here -->";
      document.getElementById("PrestigeOtherContainer").innerHTML =
...
function addResourceBlock(res) {
  ...
  if (
    !resource ||
    !childResource ||
    document.getElementById(resource.name + "Block")
  )
    return;
...
function addWorkerBlock(wor) {
  ...
  var elem =
    '<div id="' +
    worker.name +
    'Block" class="backdrop bg-white bg-opacity-10 p-2 text-white shadow-md relative">' +
    '<div id="' +
    worker.name +
    'Button" class="flex clickable bg-white bg-opacity-25 text-white p-2" ' +
    "onmouseover=\"tooltip('" +
    wor +
    ...
    wor +
    "')\">" +
    '<div class="jbCurrent text-3xl font-bold pl-4 pr-4 pt-1">' +
    '<span id="Current' +
    worker.name +
    '">' +
    prettify(worker.current, 0, true) +
...
function addBuildingBlock(bld) {
  ...
  var elem =
    '<div id="' +
    building.name +
    'Block" class="backdrop bg-white bg-opacity-10 p-2 text-white shadow-md">' +
    '<div id="' +
    building.name +
    'Button" class="flex clickable bg-white bg-opacity-25 text-white p-2" ' +
    "onmouseover=\"tooltip('" +
    ...
    bld +
    "')\">" +
    '<div class="jbCurrent text-3xl pl-4 pr-4 pt-1 font-bold">' +
    '<span id="Current' +
    building.name +
    '">' +
    prettify(building.current, 0, true) +
    "</span>" +
    "</div>" +
    '<div class="text-md flex-1 mt-2">' +
    "<span>" +
...
function addUpgradeBlock(upg) {
  ...
  if (
    !upgrade ||
    upgrade.bought ||
    document.getElementById(upgrade.name + "Block")
  )
    return;
  var specialUpgrade = "";
...
function addLeaderBlock(ldr) {
  ...
  var elem =
    ...
    '<div id="' +
    leader.name +
    'Button" class="jbButton button unclickable' +
    (leader.bought ? " advBought" : "") +
    '"' +
    "onmouseover=\"tooltip('" +
    ldr +
    '\', event)" onmouseleave="tooltip()" ' +
...
function addAdvancementBlock(adv) {
  ...
  if (!advancement || document.getElementById(advancement.name + "Block"))
    return;
...
function recalculateUpgradeCost(upgrade) {
  ...
  for (var res in upgrade.resourceCost) {
    ...
    if (typeof upgrade.goldCostBase !== "undefined")
      upgrade.goldCost = Math.ceil(
        upgrade.goldCostBase * game.player.upgradeCost
      );
  }
}
...
function recalculateTradeCost(trade) {
  ...
  for (var res in trade.resourceCost) {
    ...
    if (!costBlock.rate) costBlock.current = costBlock.base;
      ...
      costBlock.current = prettify(
        costBlock.base * Math.pow(costBlock.rate, trade.current),
        0
      );
  }
  ...
  if (!trade.goldRate) trade.goldCost = trade.goldBase;
    ...
    trade.goldCost = prettify(
      trade.goldBase * Math.pow(trade.goldRate, trade.current),
      0
    );
}
  ...
  for (var res in game.resources) {
    ...
    for (var par in game.resources[res].parent) {
      ...
      if (resource.name == parent.name) {
        ...
        resource.displayRate = prettify(
          resource.displayRate +
            (childResource.rate *
              (childResource.parent[par].gain * game.player.parentValue)) /
              (childResource.max - childResource.min),
          5
        );
      }
    }
...
function prestigeReset() {
  ...
  var carryover = {
    player: {
      ...
      advancementPoints:
        game.player.advancementPoints + game.player.nextAdvancementPoints,
      prestigePoints:
        game.player.prestigePoints + game.player.nextPrestigePoints,
  ...
  if (!load(loadString))
    message("There was a problem with resetting the game.", "warning");
  applyPrestiges();
  ...
  if (!game.achievements.achColonist.achieved && game.player.colonies > 0)
    achieve("achColonist");
  else if (!game.achievements.achPioneer.achieved && game.player.colonies >= 5)
    achieve("achPioneer");
  else if (
    !game.achievements.achLuminary.achieved &&
    game.player.colonies >= 10
  )
    achieve("achLuminary");
  else if (
    !game.achievements.achPatrician.achieved &&
    game.player.colonies >= 30
  )
    achieve("achPatrician");
}
...
function updateInterface() {
  ...
  getDefaultMenu();
  updateMenuInterface();
  // if (game.player.settingsUnlocked)
  //   document.getElementById("MenuArea").classList.remove("locked");
...
function updateMenuInterface() {
  ...
  if (!availableMenus.includes(game.settings.selectedMenu))
    game.settings.selectedMenu = "messages";
  //Clear all menus
...
function lockStatistics() {
  var elems = [
    ...
    "mnuStatsLabelTrades",
    "mnuStatsCurrentTrades",
    "mnuStatsLabelAdvancements",
    "mnuStatsCurrentAdvancements",
    "mnuStatsLabelWorkerCost",
    "mnuStatsCurrentWorkerCost",
    "mnuStatsLabelBuildingCost",
    "mnuStatsCurrentBuildingCost",
    "mnuStatsLabelLeaders",
    "mnuStatsCurrentLeaders",
    "mnuStatsLabelPrestiges",
    "mnuStatsCurrentPrestiges",
    "mnuStatsLabelAchievements",
    "mnuStatsCurrentAchievements",
  ];
  for (var i in elems) {
...
function updateStatistics() {
  ...
  setElement(
    "mnuStatsCurrentPrestiges",
    prettify(getAmountPrestiges(), 0, true)
  );
  ...
  setElement(
    "mnuStatsCurrentClicks",
    prettify(game.player.totalClicks, 0, true)
  );
  setElement("mnuStatsCurrentWorkers", prettify(getAmountWorkers(), 0, true));
  setElement(
    "mnuStatsCurrentBuildings",
    prettify(getAmountBuildings(), 0, true)
  );
  ...
  setElement(
    "mnuStatsCurrentAdvancements",
    prettify(getAmountAdvancements(), 0, true)
  );
  setElement(
    "mnuStatsCurrentClickPower",
    prettify(game.player.clickValue * 100, 2, true) + "%"
  );
  setElement(
    "mnuStatsCurrentParentValue",
    prettify(game.player.parentValue * 100, 2, true) + "%"
  );
  setElement(
    "mnuStatsCurrentWorkerCost",
    prettify(game.player.workerCost * 100, 2, true) + "%"
  );
  setElement(
    "mnuStatsCurrentBuildingCost",
    prettify(game.player.buildingCost * 100, 2, true) + "%"
  );
  setElement(
    "mnuStatsCurrentUpgradeCost",
    prettify(game.player.upgradeCost * 100, 2, true) + "%"
  );
}
...
function setElement(elementId, value) {
  ...
  if (!element) {
    message(
      "A new version was detected. Please Save your game and refresh the page to get it.",
      "warning"
    );
    return;
  }
...
function updatePrestigeButton() {
  ...
  if (game.player.prestigeResetUnlocked)
    document.getElementById("PrestigeResetButton").classList.remove("locked");
}
...
function updateTheme() {
  ...
  if (!availableThemes.includes(game.settings.selectedTheme))
    game.settings.selectedTheme = "defaultTheme";
  ...
  document.getElementById("mnuCurrentTheme").innerHTML =
    game.global.themeNames[i];
}
...
function switchTheme() {
  game.settings.selectedTheme = toggleOption(
    game.global.availableThemes,
    game.settings.selectedTheme
  );
...
function updateGraphics() {
  ...
  document.getElementById("mnuCurrentGraphics").innerHTML = game.settings
    .fancyGraphics
    ? "Fancy"
    : "Default";
}
...
function updateAvailableUpgrades(active = false) {
  ...
  if (game.upgrades.upgBucket.locked && game.resources.wood.current >= 1) {
    unlock("upgBucket");
    tutorialMessage("levels");
  } else if (
    game.upgrades.upgBarrels.locked &&
    game.resources.water.current == game.resources.water.max &&
    game.resources.wood.current >= 1
  )
    unlock("upgBarrels");
  else if (
    game.upgrades.upgShed.locked &&
    game.resources.wood.current == game.resources.wood.max
  )
    unlock("upgShed");
  else if (game.upgrades.upgMasonry.locked && game.resources.stone.current >= 1)
    unlock("upgMasonry", "upgMineCart");
  else if (
    game.upgrades.upgWaterDam.locked &&
    game.resources.water.current >= 60
  )
    unlock("upgWaterDam");
  else if (
    game.upgrades.upgIronBucket.locked &&
    game.resources.iron.current >= 1
  )
    unlock("upgIronBucket", "upgIronAxe", "upgIronPickaxe");
  else if (
    game.upgrades.upgNourishment.locked &&
    game.resources.wheat.current >= 15
  )
    unlock("upgNourishment", "upgIrrigation", "upgApprenticeship");
  else if (
    game.upgrades.upgJewelers.locked &&
    game.resources.diamond.current >= 1
  )
    unlock(
      "upgJewelers",
      "upgDiamondTrade",
  ...
  if (
    !game.achievements.achWillOfThePeople.achieved &&
    game.buildings.waterMill.current >= 20 &&
    game.buildings.grainMill.current >= 20 &&
    getAmountTrades() < 1
  )
    achieve("achWillOfThePeople");
  else if (
    !game.achievements.achPolymath.achieved &&
    getAmountTrades() >= 100 &&
    getAmountUpgrades() >= 100
  )
    achieve("achPolymath");
}
...
function updateAdvancementValues(active = false) {
  if (!active) return;
  var points =
    (game.player.advancementPoints > 0
      ? '<span style="color:greenyellow;">'
      : "<span>") +
    game.player.advancementPoints +
    " " +
    (game.player.advancementPoints > 1 ? "Points" : "Point") +
    "</span>";
  document.getElementById("AdvancementsPoints").innerHTML =
    points + " Available";
  for (var adv in game.advancements) {
    var advancement = game.advancements[adv];
    ...
    if (!advancement.locked) {
      if (canAfford(advancement)) {
        if (advButton.classList.contains("unclickable"))
          advButton.classList.replace("unclickable", "clickable");
      } else {
        if (advButton.classList.contains("clickable"))
          advButton.classList.replace("clickable", "unclickable");
      }
...
function updatePrestigeValues(active = false) {
  if (!active) return;
  var pointsIndication =
    game.player.prestigePoints > 0
      ? "<span style='color:greenyellow;'>" +
        game.player.prestigePoints +
        "</span>"
      : game.player.prestigePoints;
  document.getElementById("PrestigeCurrentPoints").innerHTML =
    pointsIndication +
    (game.player.nextPrestigePoints > 0
      ? " (<span style='color:greenyellow;'>+" +
        game.player.nextPrestigePoints +
        "</span>)"
      : "");
  for (var ptg in game.prestiges) {
    var prestige = game.prestiges[ptg];
    ...
    if (canAfford(prestige)) {
      if (ptgButton.classList.contains("unclickable"))
        ptgButton.classList.replace("unclickable", "clickable");
      ptgButton.classList.remove("ptgInProgress");
    } else {
      if (ptgButton.classList.contains("clickable"))
        ptgButton.classList.replace("clickable", "unclickable");
...
function updateAchievementValues(active = false) {
  if (!active) return;
  document.getElementById("achCurrentPoints").innerHTML =
    game.player.achievementPoints;
  for (var ach in game.achievements) {
...
function updateResValues(resource, active, resetting = false) {
  ...
  var newCurrent = game.settings.roundResources
  ...
  if (currentElem.textContent != newCurrent)
  ...
  else if (!currentBar.classList.contains("rsAnimated"))
    currentBar.classList.add("rsAnimated");
...
function updateResRateValues(resource, active) {
  if (!active || resource.locked) return;
  if (hasParent(resource)) {
    for (var parent in resource.parent)
      updateResRateValues(getFromText(parent), active);
    return;
  }
  document.getElementById(
    resource.name + "DisplayRate"
  ).textContent = getResRate(resource);
}
...
function getResRate(resource) {
  ...
  if (
    game.settings.resourceAlwaysDecimals &&
    Math.abs(resource.displayRate) < 10000
  )
...
function updateJobValues(job, active) {
  ...
  if (canAfford(job)) {
    if (jobButton.classList.contains("unclickable"))
      jobButton.classList.replace("unclickable", "clickable");
  } else {
    if (jobButton.classList.contains("clickable"))
      jobButton.classList.replace("clickable", "unclickable");
  }

  if (!active) return;
  document.getElementById("Current" + job.name).textContent = prettify(
    job.current,
    0,
    true
  );
...
function updateUpgradeValues(upgrade, active) {
  ...
  if (!upgrade.locked && !upgrade.bought) {
    if (canAfford(upgrade)) {
      if (upgButton.classList.contains("unclickable"))
        upgButton.classList.replace("unclickable", "clickable");
    } else {
      if (upgButton.classList.contains("clickable"))
        upgButton.classList.replace("clickable", "unclickable");
    }
  }
...
function updateLeaderValues(leader, active = false) {
  ...
  if (
    ...
    if (ldrButton.classList.contains("unclickable"))
      ldrButton.classList.replace("unclickable", "clickable");
  } else {
    if (ldrButton.classList.contains("clickable"))
...
function updateTradeValues(trade) {
  ...
  if (canAfford(trade)) {
    if (trdButton.classList.contains("unclickable"))
      trdButton.classList.replace("unclickable", "clickable");
  } else {
    if (trdButton.classList.contains("clickable"))
      trdButton.classList.replace("clickable", "unclickable");
  }
}
...
function updateAutosaveButton() {
  ...
  // autosaveLabel.classList.remove("autosaveON");
  // autosaveLabel.classList.remove("autosaveOFF");
  if (game.settings.autoSaveEnabled) {
    autosaveLabel.classList.add("translate-x-5");
    autosaveLabel.classList.add("bg-indigo-900");
    autosaveLabel.classList.remove("translate-x-0");
    autosaveLabel.classList.remove("bg-white");
    //document.getElementById("autosaveLabel").textContent = "Autosave"
  } else {
    autosaveLabel.classList.add("translate-x-0");
    autosaveLabel.classList.add("bg-white");
    autosaveLabel.classList.remove("translate-x-5");
    autosaveLabel.classList.remove("bg-indigo-900");
    //document.getElementById("autosaveLabel").textContent = "Autosave"
  }
}
...
function updateModal(what) {
  if (!what || game.global.currentTooltip == what) {
    ...
    updateModalBackground(false, true);
    tooltip("import");
    if (document.getElementById("importText"))
...
function updateModalBackground(hide = true, menuAllowed = false) {
  ...
  if (menuAllowed)
    document.getElementById("bgDisable").classList.add("menuAllowed");
...
function tooltip(what = "hide", event = null, repositionTooltip = true) {
  ...
  if (what == "pause") {
    ...
    bottomText =
      '<div class="mdButton mdRed button" onclick="togglePause()"><span>Unpause</span></div>';
  } else if (what == "export") {
    titleText = "Export";
    mainText =
      'Copy/Paste the following text to import your game:<textarea id="exportText" class="mdTextArea" disabled>' +
      save(true) +
      "</textarea>";
    bottomText =
      '<div class="mdButton mdGreen button" onclick="copyExport()">Copy to Clipboard</div><div class="mdButton mdGray button" onclick="toggleExport()"><span>Close</span></div>';
  } else if (what == "import") {
    titleText = "Import";
    mainText =
      'Paste the export string here:<textarea id="importText" class="mdTextArea"></textarea>';
    bottomText =
      '<div class="mdButton mdGreen button" onclick="importLoad()">Load</div><div class="mdButton mdGray button" onclick="toggleImport()">Close</div>';
  } else if (what === "delete") {
    titleText = "Delete Save";
    mainText = "Delete your save and reset the game? (This cannot be undone)";
    bottomText =
      '<div class="mdButton mdRed button" onclick="deleteSave()">Yes</div><div class="mdButton mdGray button" onclick="tooltip()">Cancel</div>';
    updateModalBackground(false, true);
  } else if (what == "colony") {
    var advPts = prettify(
      game.player.advancementPoints +
        game.player.nextAdvancementPoints +
        game.prestiges.ptgAdvanced.level * game.prestiges.ptgAdvanced.rate,
      0,
      true
    );
    var ptgPts = prettify(
    ...
    var gold = prettify(
      Math.floor(game.player.gold * game.player.nextGoldRate),
      0,
      true
    );
    titleText = "Start a new colony?";
    mainText =
      "Are you sure you want to start a new colony? Your progress will be reset, however you will be allowed to keep the following:<b><br />" +
      ptgPts +
      " Prestige Point" +
      (ptgPts > 1 ? "s" : "") +
      "<br />" +
    ...
    mainText += "</b>";
    bottomText =
      '<div class="mdButton mdGreen button" onclick="prestigeReset()">Yes</div><div class="mdButton mdGray button" onclick="tooltip()">Cancel</div>';
    ...
    if (itemType == "workers" || itemType == "buildings") {
      ...
      titleText =
        ...
        (item.level > 0 ? " (" + item.level + "/" + item.maxLevel + ")" : "");
      mainText = item.description();
      if (item.colonies > 0 && item.colonies > game.player.colonies)
        mainText +=
          "<br /><span style='color: red;'>You need to have built at least <b>" +
          item.colonies +
          (item.colonies > 1 ? " Colonies" : " Colony") +
          "</b> to purchase this upgrade.</span>";
      var tempBottomText = getCostLabel(item);
      if (tempBottomText.length < 1)
        bottomText = "<span style='color: green;'>This upgrade is free!</span>";
      else bottomText = tempBottomText;
    } else if (itemType == "advancements") {
      titleText = item.label;
      mainText = item.description();
      bottomText = item.bought
        ? "<span style='color: green;'>You have this advancement.</span>"
        : "This advancement costs <span style='color: " +
          (game.player.advancementPoints >= item.advCost ? "green" : "red") +
          ";'>" +
          item.advCost +
          "</span> Advancement " +
          (item.advCost > 1 ? "Points" : "Point") +
          ".";
      ...
      titleText =
        ...
        (item.maxLevel > 0
          ? " (" +
            Math.min(item.level + 1, item.maxLevel) +
            "/" +
            item.maxLevel +
            ")"
          : "");
      ...
      bottomText = item.bought
        ? "<span style='color: green;'>You have this prestige skill.</span>"
        : "This prestige skill costs <span style='color: " +
          (game.player.prestigePoints >= item.ptgCost ? "green" : "red") +
          ";'>" +
          item.ptgCost +
          "</span> Prestige " +
          (item.ptgCost > 1 ? "Points" : "Point") +
          ".";
    } else if (itemType == "leaders") {
      titleText = item.fullLabel ? item.fullLabel : item.label;
      mainText =
        item.description() +
        "<br/><span class='tooltipSmaller'>\"" +
        item.quote +
        '"</span>';
      if (item.bought)
        bottomText =
          "<span style='color: green;'>You hired this leader.</span>";
      else {
        bottomText = getCostLabel(item);
        if (game.player.currentLeaders >= game.player.maxLeaders)
          bottomText +=
            "<br /><span style='color:red;'>You cannot hire an additional leader.</span>";
      }
      ...
      if (item.hidden && !item.achieved) {
        ...
        titleText = item.fullLabel ? item.fullLabel : item.label;
        var progress =
          item.hasOwnProperty("progress") && !item.achieved
            ? '<br /><span class ="tooltipSmaller">' +
              item.progress() +
              "</span>"
            : "";
        ...
        if (item.achieved)
          bottomText +=
            "<span style='color: green;'> (You have this achievement.)</span>";
      }
    }
  }
...
function positionTooltip(tooltipDiv, event, isCenter) {
  ...
  var tiph = Math.max(
    tooltipDiv.clientHeight,
    tooltipDiv.scrollHeight,
...
function GetMax(what, number, resets, active) {
  ...
  if (resets && number >= what.max) {
    ...
    for (var par in what.parent) {
      var parent = getFromText(par);
      if (what.name.includes(parent.name) && parent.current != parent.max)
        toReset = true;
    }
    if (!hasParent(what) || toReset) {
      ...
      for (var par in what.parent) {
        var parent = getFromText(par);
        var parentGain = prettify(
          what.parent[par].gain * game.player.parentValue,
          2
        );
        gainResource(parent, parentGain, active);
      }
      if (game.prestiges.ptgLoopClicking.bought)
        return GetMax(what, number - what.max, resets, active);
      return what.min;
...
function hasParent(what) {
  if (!what || !what.parent || Object.keys(what.parent).length === 0)
    return false;
  return true;
}
